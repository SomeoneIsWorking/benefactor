...
| Release CI | Build and upload platform packages through the canonical Python builders, reject unbundled Windows runtime imports, and provide the asset-free browser artifact for the central Pages publisher | `.github/workflows/release.yml`, `tools/build_*.py`, `tools/check_windows_imports.py`; publication is owned by sibling `pages/` | tag/manual release workflow | `docs/project-goals.md` |
