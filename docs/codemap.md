...
| Setup screen flow | Drive the shared in-app setup screen, apply the platform's picker result, and turn an accepted set into the disks the product boots with | `src/platform/setup_flow.{h,cpp}` | `benefactor::platform::run_setup_flow`, `committed_disks` | `docs/project-state.md` |
