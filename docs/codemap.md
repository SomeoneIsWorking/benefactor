...
| Native level-data decoding | Decode the title's bounded `=SB=` LH5-style level stream into guest memory while preserving the guest JSR continuation | `src/engine/sb_decompress.c`, `src/engine/sb_decompress.h` | `sb_decompress` from the gameplay level-load override | `instructions/gameplay-engine-map.md` |
...
| Guest execution diagnostics | Turn the interpreter's retired-instruction ring into log dumps and debug-server listings | `src/port/guest_trace.c`, `src/port/guest_trace.h`, `src/port/http_debug.c` | `rt_insn_ring_*` / `rt_recent_snapshot` | `docs/issues/0007-interpreter-boundaries-and-beam-time.md` |
...
| Desktop disk setup | Native first-run file picker, complete-install validation, and OS user-data persistence | target native platform/setup owner | future desktop setup entry point | `docs/project-goals.md` |
...
| Release CI | Build and upload platform packages through the canonical Python builders; deploy browser output to Pages | `.github/workflows/release.yml`, `tools/build_*.py` | tag/manual release workflow | `docs/project-goals.md` |
