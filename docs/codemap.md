...
| Host UI | Pause, options, level selector, HUD, and touch presentation; edits config but does not own it | `src/port/pause_menu.c` (+ `pause_menu_draw.c`), `src/port/level_select_ui.c`, `src/port/hud_icons.c`, `src/port/touch_controls.cpp` | port UI calls | `README.md` |
...
| App icon | Author the product's mark once and emit every platform's form of it: desktop SVG, browser favicon, Android adaptive layers plus the pre-26 tile and a monochrome layer, Windows `.ico` and its resource statement, macOS `.icns` | `tools/draw_app_icon.py` (generator), `platforms/icons/benefactor.svg` (the mark) | `python -m tools.draw_app_icon --write` / `--check` / `--sheet` / `--android-preview` | `docs/project-state.md` |
...
| Picker report translation | Turn the private directory and document names a platform picker reports into the candidate paths the staged-set resolver validates | `src/platform/selection_report.{h,cpp}` | `benefactor::platform::selection_report_paths` | `docs/project-state.md` |
| Staged-set resolution | Resolve the three exact images or one bounded ZIP into the store's all-or-nothing publication | `src/platform/staged_disks.{h,cpp}`, `src/platform/disk_identity.{h,cpp}` | `benefactor::platform::validate_staged_disks`, `validate_set` | `docs/project-state.md` |
| Android application policy | Benefactor disk identity and promotion, picker-result hand-off, touch-action meaning, orientation, and package resources; shared Activity lifecycle, SAF staging, and touch contacts belong to `shared/android-port` | `platforms/android/` consuming the sibling `shared/android-port` runtime | `BenefactorActivity` and Gradle package | `README.md` |
| Desktop disk setup | Native first-run picker behind the shared in-app setup screen, then exact identity validation | `src/platform/desktop_setup.cpp` | `desktop_setup_disks` | `docs/project-goals.md` |
...
| Browser chooser bridge | Open the browser's own file chooser, move the chosen documents into the module's filesystem, and answer the native pick exactly once; identity, archives, and the screen stay in the product | `platforms/web/disk_setup.js` | `benefactorWebPickFiles`, `benefactorWebStatus` | `docs/project-state.md` |
...
| Pinned shared revisions | Every shared checkout the release workflow pins must name a pushed commit, because the local build resolves a sibling tree and CI authenticates the pin | `.github/workflows/release.yml` (single list of pins), `tools/pinned_revisions.py` | `require_pinned_revisions`, `python -m tools.pinned_revisions` | `AGENTS.md` |
