...
| When a frame is over | The beam derived from guest time, the boundary that crossing it raises, and holding that boundary until the guest reaches its own wait | `src/engine/hw_beam.c`, `src/engine/hw_private.h` | `hw_vblank_wait`, `hw_boundary_hold`, `hw_beam_wait_below/_above` | `docs/issues/0008` |
