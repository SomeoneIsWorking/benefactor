...
2. **Removed Non-Subroutine Override `$59C5B0`**:
   ...
   *Superseded 2026-09-16.* Removing the registration also removed the feature the override existed for — the game-over bypass, which sends a death back to the level card instead of the CONTINUE/GAME OVER menu. It was gone for months and nothing noticed. The defect was never the override, only its pass-through: `rt_call` gave a branch target a subroutine frame it never had. The override is registered again and completes with `rt_continue_original`, the boundary for a tail-entered observer, which resumes the guest's own instruction in the run already in progress and pushes nothing. `tools/menu_soak.py` now runs the player out of lives every fifth round and fails if the game-over markers in `$1093` are still set afterwards; with the registration removed again it fails in that round, and the level card transition this issue is about still completes.
