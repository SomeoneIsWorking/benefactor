...
2. **Unified Call Architecture**: Having distinct `rt_call_original` and `rt_call_original_subroutine` primitives created confusion between host dispatch and interpreted dispatch. The architecture requires a single unified `rt_call` where both the interpreter and host dispatch share identical "if overridden and not suppressed, call override; otherwise call guest" logic. In `amigaport::Executor::call`, when an override calls its own address (`is_active`), it automatically activates scoped suppression and inspects the calling instruction: if entered as a subroutine (`JSR`/`BSR`) rather than a tail jump (`JMP`/`BRA`), it resolves the return address from the stack (`A7`) and halts at `rts` via `stop_at_pc`.
...
3. Refactored `amigaport::Executor::call` to handle unified override dispatch, scoped self-suppression, and automatic subroutine return address detection (`stop_at_pc`).
