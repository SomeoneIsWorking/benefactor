...
| S008 | Optional platformer physics provides variable jump, air control, momentum, and tunable motion while classic physics remains | partial | S005; issue 0010 fixed subroutine fallback boundaries in `src/port/overrides/platformer.c`, restoring classic jump trajectories, falling, landing, directional hopping, and long jumps to match the reference oracle | G002 |
