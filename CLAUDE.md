...
- **Vestigial password field — FIXED (2026-06-11)** by owning its renderer `$003DAA` (native_menu_pwfield_draw, allow-listed in rt.c). The "inline fall-through" theory was FALSE (the inline copy is unreachable double-emission). CAUTION: `$3DAA` is NOT draw-only — it also copies the current char into the `-$1380(a5)` text shadow that the PLAY GAME fire path decodes (password = start state); the override keeps that write and skips only the pixels. The menu art ships a CLEAN field area (verified) — no erase machinery exists anymore.
