...
#include "runtime/guest_runtime.h" /* active amigaport memory view; no SDL dependency */
#include <stdio.h>
#include <string.h>
...
int disk_boot_open(const char *const *paths, int n) {
    ...
    for (int i = 0; i < n && i < MAX_DISKS; i++) {
        ...
        rewind(s_disk[i]);
        benefactor_log_write(BENEFACTOR_LOG_DEBUG, "disk", "opened[%d] '%s' size=%u", i, paths[i],
                             s_disk_sz[i]);
        opened++;
    }
    return opened > 0 ? 0 : -1;
...
uint32_t atn_decrunch(uint32_t start) {
    ...
    for (;;) {
        ...
        if (atn_getbit(&b) == 1) {
            int d1b = (d1 << 1) & 0xFFFF;
            if (atn_getbit(&b) == 1) {
                a2base = (tbl[8 + d1b] << 8) | tbl[8 + d1b + 1];
                d0 += 8;
            } else {
                a2base = (tbl[d1b] << 8) | tbl[d1b + 1];
                d0 += 4;
            }
        }
        nb = tbl[0x10 + d0];
