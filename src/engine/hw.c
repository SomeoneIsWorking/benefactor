 ...
 * by pc_state_reset_defaults() in src/port/game_loop.c. */

/* Current interrupt-enable shadow (INTENA, $DFF09A). The IRQ-delivery code in
 * The game-loop module uses this to fire a level's handler ONLY when the game has that level
 ...
 * width = 352 (no change; harness/standalone identical). Set widescreen=<width>,
...
static void hw_compose_output(void) {
     ...
     * widescreen_compare forces the BenRen compose even at 352 so it can be diffed
     ...
     *   compare at 352 (cmp)         → full native re-render, engine-aligned, diffed vs s_fb.
 ...
 * (guest engine under the interpreter + audio, measured by pc_step), the native render, the
...
static void apply_bound_input(void) {
    ...
    benefactor_log_write(BENEFACTOR_LOG_TRACE, "input", "apply bound input (right was %d)",
                         s_joy_right);
     ...
     * gameplay; outside gameplay it falls through to exit(). */
    {
        ...
        if (pc_pause_active()) {
            ...
            case SDLK_RETURN:
                ...
                return;
            default:
                return; /* swallow other keys while paused */
            }
        }
    }
    ...
    /* Title-menu LEVEL SELECT panel: arrows navigate the panel, not the game. */
    {
        ...
        if (down && g_level_select_visible &&
            ...
            case SDLK_DOWN:
                ...
                break;
            case SDLK_LEFT:
                pc_profile_try_select(pc_world_first_level(w > 0 ? w - 1 : 0));
                break;
            case SDLK_RIGHT:
                pc_profile_try_select(pc_world_first_level(w < PC_NUM_WORLDS - 1 ? w + 1 : w));
                break;
            }
            return;
        }
 ...
 * legacy widescreen pixel width (read in hw_init) stays in effect. */
...
void hw_widescreen_refresh(void) {
    ...
    case 0:
        w = hw_width_43();
        break; /* disabled = 4:3 symmetric crop */
    case 1:
        w = HW_DISPLAY_H * 16 / 9;
        break;
    case 2:
...
int hw_benren_active(void) {
    ...
    return (m == PC_RENDER_BENREN) || (m == PC_RENDER_AUTO && s_hw_out_w > HW_DISPLAY_W);
}
...
int hw_handle_sdl_event(const SDL_Event *ev) {
    ...
    case SDL_CONTROLLERAXISMOTION: {
        ...
        for (int dir = 0; dir < 2; dir++) { /* 0 = negative, 1 = positive */
            int mag = dir ? v : -v;
            int on =
                s_axis_on[axis][dir] ? (mag > PAD_AXIS_OFF_THRESH) : (mag > PAD_AXIS_ON_THRESH);
...
int hw_present_frame(void) {
    ...
    /* Harness snap is taken by the caller in src/port/game_loop.c AFTER the timer interrupt,
     ...
     * (out + in, 1s total — see pc_freecam_toggle). */
    {
        ...
        if (fa > 0) {
            ...
            for (int i = 0; i < n; i++) {
                uint32_t px = s_out[i];
                s_out[i] = 0xFF000000u | ((((px >> 16) & 0xFF) * (uint32_t)keep / 255u) << 16) |
    ...
    if (!s_headless) {
        ...
        if (hw_scene_render_enabled() && s_backend->present_scene && native_render_scene_ready() &&
            !overlay) {
            int ylo, yhi;
            native_render_scene_yrange(&ylo, &yhi);
            s_backend->present_scene(native_render_scene(), ylo, yhi, s_out, s_hw_out_w,
                                     HW_DISPLAY_H, s_ovl_rects, s_ovl_nrects);
        } else {
    ...
    /* ── Test mode hooks (env vars) ──────────────────────────────────────── */
    {
        ...
        if (press_frame >= 0 && (uint32_t)s_frame_num == (uint32_t)press_frame) {
            hw_set_fire(1);
            hw_set_mouse_lmb(1);
            benefactor_log_write(BENEFACTOR_LOG_INFO, "test", "fire pressed at frame %d",
                                 s_frame_num);
        }
        if (release_frame >= 0 && (uint32_t)s_frame_num == (uint32_t)release_frame) {
            hw_set_fire(0);
            hw_set_mouse_lmb(0);
            benefactor_log_write(BENEFACTOR_LOG_INFO, "test", "fire released at frame %d",
                                 s_frame_num);
        }
...
/* ── Harness frame hook (NULL by default; set by the separate oracle harness) ─────── */
...
#define _NREGS 14
        ...
        static const uint16_t EXT_REG[7] = {0x00EA, 0x00EC, 0x00EE, 0x00F0, 0x00F2, 0x00F4, 0x00F6};
...
#define _COP_REG(idx, fallback)                                                                    \
    (pre_set[idx] ? pre_val[idx] : (post_set[idx] ? post_val[idx] : (fallback)))
...
#undef _NREGS
    ...
    for (int n = 0; n < 4; n++) {
        ...
        s->audio[n].lc = ((uint32_t)s_regs[(b + 0) >> 1] << 16) | s_regs[(b + 2) >> 1];
    ...
    s->flip_flop =
        g_mem ? (uint16_t)((((uint8_t *)g_mem)[0x41A2] << 8) | ((uint8_t *)g_mem)[0x41A3]) : 0;
    s->sprite_ctr =
        g_mem ? (uint16_t)((((uint8_t *)g_mem)[0x42FC] << 8) | ((uint8_t *)g_mem)[0x42FD]) : 0;
    {
...
uint8_t hw_read8(uint32_t addr) {
    ...
    if ((addr >= 0xBFD000 && addr <= 0xBFDFFF) || (addr >= 0xBFE000 && addr <= 0xBFEFFF)) {
...
uint16_t hw_read16(uint32_t addr) {
     ...
     * interpreter cold-start coroutine (via the hook's longjmp) so the frame-driven loop
    ...
    if (addr >= 0xBFD000 && addr <= 0xBFDFFF) {
        ...
        switch (reg) {
        case CIA_TALO:
            return s_ciab_ta_cnt & 0xFF;
        case CIA_TAHI:
            return s_ciab_ta_cnt >> 8;
        case CIA_ICR: {
                ...
                val &= ~0x80;
            {
                ...
                if (k == n && n < 64 && g_gameplay_active) {
                    ...
                    benefactor_log_write(BENEFACTOR_LOG_TRACE, "cia",
                                         "$BFE001 -> %02X from guest pc=$%06X fire=%d", val, pc,
                                         s_fire_pressed);
                }
            }
            return val;
    ...
    if (addr >= 0xDFF000 && addr <= 0xDFFFFF) {
        ...
        case JOY0DAT: {
            ...
            HW_LOG("JOY0DAT read -> $%04X (up=%d dn=%d lt=%d rt=%d)\n", v, s_joy_up, s_joy_down,
                   s_joy_left, s_joy_right);
            return v;
        }
        case JOY1DAT:
            return hw_joystick(); /* player joystick is on port 1 (the gameplay reads $c(a6)) */
        case POT0DAT:
        case POT1DAT: {
            uint16_t v = 0;
            if (s_fire_pressed)
...
void hw_write16(uint32_t addr, uint16_t v) {
    ...
    if (addr >= 0xBFD000 && addr <= 0xBFDFFF) {
        ...
        switch (reg) {
        case CIA_TALO:
            s_ciab_ta_latch = (s_ciab_ta_latch & 0xFF00) | (v & 0xFF);
            break;
        case CIA_TAHI:
    ...
    if (addr >= 0xDFF000 && addr <= 0xDFFFFF) {
        ...
        case BPL1PTL:
            s_regs[BPL1PTL >> 1] = v;
            s_bplptr[0] = _bplptr(BPL1PTH, BPL1PTL);
            HW_LOG("BPL1PT = $%06X\n", s_bplptr[0]);
            HWTRACE("[HWTRACE] BPL1PT=$%06X (lo write=$%04X)\n", s_bplptr[0], v);
            break;
        case BPL1PTH:
            s_regs[BPL1PTH >> 1] = v;
            s_bplptr[0] = _bplptr(BPL1PTH, BPL1PTL);
            HW_LOG("BPL1PTH write -> BPL1PT = $%06X\n", s_bplptr[0]);
            HWTRACE("[HWTRACE] BPL1PT=$%06X (hi write=$%04X)\n", s_bplptr[0], v);
            break;
        case BPL2PTL:
            s_regs[BPL2PTL >> 1] = v;
            s_bplptr[1] = _bplptr(BPL2PTH, BPL2PTL);
            HW_LOG("BPL2PT = $%06X\n", s_bplptr[1]);
            HWTRACE("[HWTRACE] BPL2PT=$%06X (lo write=$%04X)\n", s_bplptr[1], v);
            break;
        case BPL2PTH:
            s_regs[BPL2PTH >> 1] = v;
            s_bplptr[1] = _bplptr(BPL2PTH, BPL2PTL);
            HW_LOG("BPL2PTH write -> BPL2PT = $%06X\n", s_bplptr[1]);
            HWTRACE("[HWTRACE] BPL2PT=$%06X (hi write=$%04X)\n", s_bplptr[1], v);
            break;
        case BPL3PTL:
            s_regs[BPL3PTL >> 1] = v;
            s_bplptr[2] = _bplptr(BPL3PTH, BPL3PTL);
            HW_LOG("BPL3PT = $%06X\n", s_bplptr[2]);
            HWTRACE("[HWTRACE] BPL3PT=$%06X (lo write=$%04X)\n", s_bplptr[2], v);
            break;
        case BPL3PTH:
            s_regs[BPL3PTH >> 1] = v;
            s_bplptr[2] = _bplptr(BPL3PTH, BPL3PTL);
            HW_LOG("BPL3PTH write -> BPL3PT = $%06X\n", s_bplptr[2]);
            HWTRACE("[HWTRACE] BPL3PT=$%06X (hi write=$%04X)\n", s_bplptr[2], v);
            break;
        case BPL4PTL:
            s_regs[BPL4PTL >> 1] = v;
            s_bplptr[3] = _bplptr(BPL4PTH, BPL4PTL);
            HW_LOG("BPL4PT = $%06X\n", s_bplptr[3]);
            HWTRACE("[HWTRACE] BPL4PT=$%06X (lo write=$%04X)\n", s_bplptr[3], v);
            break;
        case BPL4PTH:
            s_regs[BPL4PTH >> 1] = v;
            s_bplptr[3] = _bplptr(BPL4PTH, BPL4PTL);
            HW_LOG("BPL4PTH write -> BPL4PT = $%06X\n", s_bplptr[3]);
            HWTRACE("[HWTRACE] BPL4PT=$%06X (hi write=$%04X)\n", s_bplptr[3], v);
            break;
        case BPL5PTL:
            s_regs[BPL5PTL >> 1] = v;
            s_bplptr[4] = _bplptr(BPL5PTH, BPL5PTL);
            HW_LOG("BPL5PT = $%06X\n", s_bplptr[4]);
            break;
        case BPL5PTH:
            s_regs[BPL5PTH >> 1] = v;
            s_bplptr[4] = _bplptr(BPL5PTH, BPL5PTL);
            HW_LOG("BPL5PTH write -> BPL5PT = $%06X\n", s_bplptr[4]);
            break;
        case BPL6PTL:
            s_regs[BPL6PTL >> 1] = v;
            s_bplptr[5] = _bplptr(BPL6PTH, BPL6PTL);
            HW_LOG("BPL6PT = $%06X\n", s_bplptr[5]);
            break;
        case BPL6PTH:
            s_regs[BPL6PTH >> 1] = v;
            s_bplptr[5] = _bplptr(BPL6PTH, BPL6PTL);
            HW_LOG("BPL6PTH write -> BPL6PT = $%06X\n", s_bplptr[5]);
            break;
        /* Sprite pointers */
        case SPR0PTL:
            s_regs[SPR0PTL >> 1] = v;
            s_sprpt[0] = _bplptr(SPR0PTH, SPR0PTL);
            break;
        case SPR0PTH:
            s_regs[SPR0PTH >> 1] = v;
            s_sprpt[0] = _bplptr(SPR0PTH, SPR0PTL);
        ...
        case SPR1PTL:
            s_regs[SPR1PTL >> 1] = v;
            s_sprpt[1] = _bplptr(SPR1PTH, SPR1PTL);
            break;
        case SPR1PTH:
            s_regs[SPR1PTH >> 1] = v;
            s_sprpt[1] = _bplptr(SPR1PTH, SPR1PTL);
        ...
        case SPR2PTL:
            s_regs[SPR2PTL >> 1] = v;
            s_sprpt[2] = _bplptr(SPR2PTH, SPR2PTL);
            break;
        case SPR2PTH:
            s_regs[SPR2PTH >> 1] = v;
            s_sprpt[2] = _bplptr(SPR2PTH, SPR2PTL);
        ...
        case SPR3PTL:
            s_regs[SPR3PTL >> 1] = v;
            s_sprpt[3] = _bplptr(SPR3PTH, SPR3PTL);
            break;
        case SPR3PTH:
            s_regs[SPR3PTH >> 1] = v;
            s_sprpt[3] = _bplptr(SPR3PTH, SPR3PTL);
        ...
        case SPR4PTL:
            s_regs[SPR4PTL >> 1] = v;
            s_sprpt[4] = _bplptr(SPR4PTH, SPR4PTL);
            break;
        case SPR4PTH:
            s_regs[SPR4PTH >> 1] = v;
            s_sprpt[4] = _bplptr(SPR4PTH, SPR4PTL);
        ...
        case SPR5PTL:
            s_regs[SPR5PTL >> 1] = v;
            s_sprpt[5] = _bplptr(SPR5PTH, SPR5PTL);
            break;
        case SPR5PTH:
            s_regs[SPR5PTH >> 1] = v;
            s_sprpt[5] = _bplptr(SPR5PTH, SPR5PTL);
        ...
        case SPR6PTL:
            s_regs[SPR6PTL >> 1] = v;
            s_sprpt[6] = _bplptr(SPR6PTH, SPR6PTL);
            break;
        case SPR6PTH:
            s_regs[SPR6PTH >> 1] = v;
            s_sprpt[6] = _bplptr(SPR6PTH, SPR6PTL);
        ...
        case SPR7PTL:
            s_regs[SPR7PTL >> 1] = v;
            s_sprpt[7] = _bplptr(SPR7PTH, SPR7PTL);
            break;
        case SPR7PTH:
            s_regs[SPR7PTH >> 1] = v;
            s_sprpt[7] = _bplptr(SPR7PTH, SPR7PTL);
