/* overlay_load.c — pure runtime-image loaders (see overlay_load.h).
 *
 * Disk read, ATN! decrunch, and relocation remain one source of truth across
 * product and oracle composition. This unit has no CPU-execution, game-state,
 * or SDL dependency. */
...
void overlay_load_main(void) {
    ...
    benefactor_log_write(BENEFACTOR_LOG_DEBUG, "overlay",
                         "main image load got=%d magic@3000=$%08X (ATN!=$41544E21)", got, m);
...
void overlay_load_gameplay(void) {
    ...
    loader_block_copy();
    for (int i = 0; i < 3; i++) {
        disk_boot_load(1, ch[i].off, ch[i].dst, ch[i].len);
     ...
     * blocks of [count, base-adjust, offsets...]; *(a4+off) += running base. */
    {
        ...
        for (int pass = 0; pass < 2; pass++) {
            ...
            for (int blk = 0; blk < 2; blk++) {
                ...
                d1 += RD32(a0);
                a0 += 4;
                for (uint32_t k = 0; k <= d7; k++) {
                    uint32_t off = RD32(a0);
                    a0 += 4;
