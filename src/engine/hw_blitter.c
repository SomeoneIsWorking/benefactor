/*
 * hw_blitter.c  –  Amiga OCS blitter simulation
 *
...
typedef struct {
    ...
    int w, h;
    char reason;
    uint32_t fn; /* guest routine that issued the blit */
} BlitLogRec;
static BlitLogRec s_blitlog[BLIT_CAP_MAX];
int s_blitlog_n = 0;
...
static void hw_do_line(uint16_t bltcon0, uint16_t bltcon1, uint16_t bltsize) {
    ...
    for (int i = 0; i < length; i++) {
        ...
        if ((!sign && (bltcon1 & BLT1_SUD)) || !(bltcon1 & BLT1_SUD)) {
            int dec = (!sign && (bltcon1 & BLT1_SUD)) ? (bltcon1 & BLT1_SUL) : (bltcon1 & BLT1_AUL);
 ...
 * active guest-call address matches — used to confirm which routine draws a given
...
void hw_do_blit(void) {
     ...
     * scroll sources from the high tile-gfx region ($5Exxxx) — exclude those. */
    {
        ...
        char why = !s_blitcap_on                   ? 'o'
                   : !used                         ? 'u'
                   : !dest_in_pages                ? 'p'
                   : !(gsrc < 0x400000u)           ? 'g'
                   : (s_blitcap_n >= BLIT_CAP_MAX) ? 'f'
                                                   : 'C';
        if (s_blitlog_n < BLIT_CAP_MAX) {
            ...
            L->h = height;
            L->reason = why;
            L->fn = rt_get_active_call_address();
        }
    ...
    if (pc_cfg_bool("blit_log", 0)) {
        ...
        if (bl) {
            fprintf(bl,
                    ...
                    "bmod=%d cmod=%d dmod=%d afwm=%04X alwm=%04X fn=%06X\n",
                    n++, apt, bpt, cpt, dpt, width_words, height, bltcon0, bltcon1, amod, bmod,
                    cmod, dmod, afwm, alwm, (unsigned)rt_get_active_call_address());
            fflush(bl);
        }
    }

    /* Report each newly observed gameplay destination region at trace level. */
    static int seen[256] = {0};
    uint32_t reg = (dpt >> 16) & 0xFF;
    if (((bltcon0 >> 8) & 1) && !seen[reg]) {
        seen[reg] = 1;
        benefactor_log_write(BENEFACTOR_LOG_TRACE, "blitter",
                             "destination region $%02X0000 (dpt=$%06X bltsize=%04X)", reg, dpt,
                             bltsize);
    }
    ...
    /* Per-blit artifact trace (blit_trace_all=true) — checksum accumulated only when on,
     * so the native port pays nothing when tracing is disabled. */
    static int s_bta_en = -1;
    static FILE *s_bta_f = NULL;
    if (s_bta_en < 0) {
        s_bta_en = pc_cfg_bool("blit_trace_all", 0);
        if (s_bta_en) {
            char d[256];
            pc_cfg_string("blit_trace_dir", "logs", d, sizeof d);
            char p[300];
            snprintf(p, sizeof p, "%s/pc_blit_trace.txt", d);
            s_bta_f = fopen(p, "w");
        }
    }
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width_words; x++) {
            ...
            if (fill_on) {
                ...
                int il = (fc ? 1 : 0) | incl;
                uint8_t flo = s_filltable[d & 0xFF][il][0];
                fc = s_filltable[d & 0xFF][il][1];
                int ih = (fc ? 1 : 0) | incl;
                uint8_t fhi = s_filltable[(d >> 8) & 0xFF][ih][0];
                fc = s_filltable[(d >> 8) & 0xFF][ih][1];
...
#ifdef HARNESS_BUILD
                if ((da >= 0x86CCu && da < 0x88CCu) || (da >= 0x7C80u && da < 0x7CA0u) ||
                    (da >= 0x04F6F4u && da < 0x0546F4u) || (da >= 0x070930u && da < 0x075930u) ||
                    (da >= 0x04F71Cu && da < 0x05471Cu) || (da >= 0x025370u && da < 0x02A370u)) {
     ...
     * comparison against the PUAE side to find the first diverging blit. */
    {
        if (s_bta_en && s_bta_f) {
            ...
            fprintf(s_bta_f,
                    ...
                    _bplptr_from(_BLTBPTH, _BLTBPTL), _bplptr_from(_BLTCPTH, _BLTCPTL), bltcon0,
                    bltcon1, bltsize, afwm, alwm, blit_init_prev_a, blit_cksum);
        }
    }
     ...
     * the pointers — e.g. guest title routine $003330's cover-art draw
     * ($3424 then $3430) writes BLTSIZE twice while loading BLTAPT/BLTDPT once,
    ...
    if (use_a) {
        s_regs[_BLTAPTH >> 1] = (uint16_t)((apt >> 16) & 0xFFFF);
        s_regs[_BLTAPTL >> 1] = (uint16_t)(apt & 0xFFFF);
    }
    if (use_b) {
        s_regs[_BLTBPTH >> 1] = (uint16_t)((bpt >> 16) & 0xFFFF);
        s_regs[_BLTBPTL >> 1] = (uint16_t)(bpt & 0xFFFF);
    }
    if (use_c) {
        s_regs[_BLTCPTH >> 1] = (uint16_t)((cpt >> 16) & 0xFFFF);
        s_regs[_BLTCPTL >> 1] = (uint16_t)(cpt & 0xFFFF);
    ...
    if (use_d) {
        s_regs[_BLTDPTH >> 1] = (uint16_t)((dpt >> 16) & 0xFFFF);
