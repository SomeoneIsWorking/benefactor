...
static int call_activity_picker(const char *message, char *error, size_t error_capacity) {
    ...
    if (!activity_class || !method || !reason) {
        snprintf(error, error_capacity, "Android Activity does not provide Benefactor disk setup");
    } else {
...
static int call_activity_void(const char *method_name, char *error, size_t error_capacity) {
    ...
    jmethodID method =
        activity_class
            ? (*environment)->GetMethodID(environment, activity_class, method_name, "()V")
            : NULL;
    if (!activity_class || !method) {
        snprintf(error, error_capacity, "Android Activity does not provide %s()", method_name);
    } else {
        (*environment)->CallVoidMethod(environment, activity, method);
        if ((*environment)->ExceptionCheck(environment)) {
            (*environment)->ExceptionClear(environment);
            snprintf(error, error_capacity, "Android Activity failed while calling %s",
                     method_name);
        }
    }
    if (activity_class)
...
static int call_activity_commit(const char *staging_root, char *installed_root,
    ...
    jmethodID method = activity_class
                           ? (*environment)
                                 ->GetMethodID(environment, activity_class, "commitBenefactorDisks",
    ...
    if (!activity_class || !method || !staging) {
        ...
        result = (jstring)(*environment)->CallObjectMethod(environment, activity, method, staging);
        if ((*environment)->ExceptionCheck(environment)) {
            (*environment)->ExceptionClear(environment);
            snprintf(error, error_capacity, "Android could not retain the validated disk set");
        } else if (!result) {
            snprintf(error, error_capacity, "Android refused the selected disk set");
        } else {
            const char *value = (*environment)->GetStringUTFChars(environment, result, NULL);
            if (!value || !copy_text(installed_root, installed_capacity, value)) {
                snprintf(error, error_capacity, "Android returned an invalid installed disk path");
            }
            if (value)
...
static int validate_disks(const char *root, const char **disks, size_t capacity) {
    if (!root || !*root || capacity < ANDROID_DISK_COUNT)
        return 0;
    for (int index = 0; index < ANDROID_DISK_COUNT; ++index) {
        int written =
            snprintf(s_disks[index], sizeof s_disks[index], "%s/Disk.%d", root, index + 1);
...
int android_bridge_select_disks(const char **disks, size_t capacity) {
    ...
    if (!ensure_import_state()) {
        benefactor_log_write(BENEFACTOR_LOG_ERROR, "android",
                             "cannot create import synchronization state: %s", SDL_GetError());
        return 0;
    }
    char installed_root[ANDROID_PATH_CAPACITY];
    int installed_written =
        snprintf(installed_root, sizeof installed_root, "%s/benefactor-disks", private_root);
    if (installed_written < 0 || (size_t)installed_written >= sizeof installed_root) {
        benefactor_log_write(BENEFACTOR_LOG_ERROR, "android", "app-private disk path is too long");
        return 0;
    }
    ...
    if (!call_activity_picker("Choose the folder containing your original "
        ...
        benefactor_log_write(BENEFACTOR_LOG_ERROR, "android", "%s", call_error);
        return 0;
    }
    ...
    SDL_UnlockMutex(s_import.mutex);
    if (import_error[0]) {
        benefactor_log_write(BENEFACTOR_LOG_ERROR, "android", "disk setup failed: %s",
                             import_error);
        return 0;
    }
    if (!validate_disks(staging_root, disks, capacity)) {
        benefactor_log_write(BENEFACTOR_LOG_ERROR, "android",
                             "disk setup failed: the selected folder must contain readable "
                             "Disk.1, Disk.2, and Disk.3 files");
        return 0;
    }
    char commit_error[512] = "";
    if (!call_activity_commit(staging_root, installed_root, sizeof installed_root, commit_error,
                              sizeof commit_error) ||
...
int android_bridge_enforce_window_policy(void) {
    ...
    if (call_activity_void("enforceBenefactorWindowPolicy", error, sizeof error))
        return 1;
    benefactor_log_write(BENEFACTOR_LOG_ERROR, "android", "%s", error);
    return 0;
}
...
Java_io_github_someoneisworking_benefactor_BenefactorActivity_nativeDiskDirectoryResult(
    ...
    if (s_import.waiting) {
        if (directory) {
            const char *value = (*environment)->GetStringUTFChars(environment, directory, NULL);
            if (value) {
