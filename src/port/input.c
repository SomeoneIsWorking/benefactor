...
typedef struct {
    int n;
    int sym[MAX_KEYS_PER_CHORD];
} Chord;
typedef struct {
    int n;
    Chord chord[MAX_CHORDS];
} Binding;
...
static const struct {
    const char *name;
    int sym;
} k_names[] = {{"Up", SDLK_UP},        {"Down", SDLK_DOWN},     {"Left", SDLK_LEFT},
               {"Right", SDLK_RIGHT},  {"Space", SDLK_SPACE},   {"Return", SDLK_RETURN},
               {"Enter", SDLK_RETURN}, {"LShift", SDLK_LSHIFT}, {"RShift", SDLK_RSHIFT},
...
const char *pc_input_code_name(int dev, int code) {
    static char buf[32];
    return (dev == PI_DEV_PAD) ? pad_name(code, buf, sizeof buf) : key_name(code, buf, sizeof buf);
}
...
static const struct {
    int act;
    const char *key[PI_BIND_DEV];
    const char *def[PI_BIND_DEV];
} k_defaults[] = {
...
void pc_input_load(void) {
    ...
    for (int d = 0; d < PI_BIND_DEV; d++) {
        ...
        for (int i = 0; i < (int)(sizeof k_defaults / sizeof k_defaults[0]); i++) {
            ...
            /* MODERN scheme: TWO buttons only (user, 2026-06-13). The primary
             * (pad A / Space) is JUMP/CONFIRM = plain fire — fire+direction is
             * the (long) jump, exactly vanilla; UP is the hop. INTERACT is the
...
const char *pc_input_action_name(int action) {
    static const char *names[PI_NUM] = {"LEFT", "RIGHT",    "UP",   "DOWN",         "JUMP",
                                        "FIRE", "INTERACT", "DROP", "FAST FORWARD", "FREE CAM"};
