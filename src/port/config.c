...
void pc_config_load(void) {
  ...
  if (s_cfg) {
    size_t got = fread(s_cfg, 1, (size_t)n, f);
    s_cfg[got] = 0;
  }
  fclose(f);
  if (s_cfg)
...
static const struct {
  const char *key, *desc;
    ...
    {"modern_controls",
     "legacy: default for BOTH per-device modern flags (bool)"},
    {"modern_controls_keyboard",
     "modern controls on the keyboard: X=interact, jump button (bool)"},
    {"modern_controls_controller", "modern controls on the controller (bool)"},
    {"interact_extend",
     "extra horizontal pickup/interact reach, px (0 = vanilla)"},
    {"widescreen", "widescreen output width, px (0 = native 352)"},
    {"widescreen_mode",
     "widescreen preset: disabled | 16:9 | ultrawide | auto (window aspect)"},
    ...
    {"fx_shadow",
     "GPU effect (Hardware): drop shadow behind characters (bool)"},
};
int pc_cfg_count(void) {
  return (int)(sizeof s_cfg_decl / sizeof s_cfg_decl[0]);
}
const char *pc_cfg_key(int i) {
  return (i >= 0 && i < pc_cfg_count()) ? s_cfg_decl[i].key : NULL;
}
...
int pc_modern_kb(void) {
  return pc_cfg_bool("modern_controls_keyboard",
                     pc_cfg_bool("modern_controls", 0));
}
int pc_modern_pad(void) {
  return pc_cfg_bool("modern_controls_controller",
                     pc_cfg_bool("modern_controls", 0));
}
int pc_modern_touch(void) {
  return pc_cfg_bool("modern_controls_touch",
                     pc_cfg_bool("modern_controls", 0));
}
int pc_modern_any(void) {
  return pc_modern_kb() || pc_modern_pad() || pc_modern_touch();
}
...
PcRenderMode pc_render_mode(void) {
  ...
  if (pc_cfg_show("renderer", buf, sizeof buf, &src) && *buf) {
    ...
    if (!warned) {
      ...
      fprintf(
          stderr,
          "[render] unknown renderer '%s' (use vanilla|benren); using auto\n",
          buf);
    }
  }
