...
void blend(uint32_t *pixel, uint32_t color, unsigned alpha) {
    ...
    const unsigned r = (((old >> 16) & 0xFFU) * inverse + ((color >> 16) & 0xFFU) * alpha) / 255U;
    const unsigned g = (((old >> 8) & 0xFFU) * inverse + ((color >> 8) & 0xFFU) * alpha) / 255U;
    const unsigned b = ((old & 0xFFU) * inverse + (color & 0xFFU) * alpha) / 255U;
    *pixel = 0xFF000000U | (r << 16) | (g << 8) | b;
}

void circle(uint32_t *argb, int width, int height, float center_x, float center_y, float radius) {
...
void disc(uint32_t *argb, int width, int height, float center_x, float center_y, float radius,
...
void line(uint32_t *argb, int width, int height, int x0, int y0, int x1, int y1, int thickness) {
...
void triangle(uint32_t *argb, int width, int height, int x0, int y0, int x1, int y1, int x2, int y2,
              uint32_t color, unsigned alpha) {
...
void direction_button(uint32_t *argb, int width, int height, float center_x, float center_y,
                      Direction direction) {
    constexpr float radius = 0.055F;
    const int cx = static_cast<int>(center_x * width);
    const int cy = static_cast<int>(center_y * height);
    const int span = std::max(11, static_cast<int>(radius * std::min(width, height) * 0.55F));
    disc(argb, width, height, center_x, center_y, radius, 0xFF101820U, 145U);
    circle(argb, width, height, center_x, center_y, radius);
    switch (direction) {
    case Direction::up:
        triangle(argb, width, height, cx, cy - span, cx - span, cy + span / 2, cx + span,
                 cy + span / 2, 0xFFFFFFFFU, 230U);
        break;
    case Direction::down:
        triangle(argb, width, height, cx, cy + span, cx - span, cy - span / 2, cx + span,
                 cy - span / 2, 0xFFFFFFFFU, 230U);
        break;
    case Direction::left:
        triangle(argb, width, height, cx - span, cy, cx + span / 2, cy - span, cx + span / 2,
                 cy + span, 0xFFFFFFFFU, 230U);
        break;
...
void control_glyphs(uint32_t *argb, int width, int height) {
    ...
    line(argb, width, height, fire_x - fire_span, fire_y + fire_span, fire_x + fire_span, fire_y,
         unit);
    line(argb, width, height, fire_x + fire_span, fire_y, fire_x - fire_span, fire_y - fire_span,
         unit);
    line(argb, width, height, fire_x - fire_span, fire_y - fire_span, fire_x - fire_span,
    ...
    if (pc_modern_touch()) {
        ...
        line(argb, width, height, interact_x + interact_span, interact_y, interact_x,
             interact_y + interact_span, unit);
        line(argb, width, height, interact_x, interact_y + interact_span,
             interact_x - interact_span, interact_y, unit);
        line(argb, width, height, interact_x - interact_span, interact_y, interact_x,
             interact_y - interact_span, unit);
