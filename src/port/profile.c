...
static const char *profile_path(void) {
    ...
    pc_cfg_string("profile", "profile.json", path, (int)sizeof path);
    return path;
}
...
static void profile_save(void) {
    ...
    if (!f) {
        benefactor_log_write(BENEFACTOR_LOG_INFO, "profile", "[profile] cannot write %s\n",
                             profile_path());
        return;
    }
    fprintf(f, "{\n  \"completed\": [");
...
void pc_profile_mark_completed(int level) {
    ...
    benefactor_log_write(BENEFACTOR_LOG_INFO, "profile",
                         "[profile] level %d completed (highest %d)\n", level,
                         pc_profile_highest_completed());
}
