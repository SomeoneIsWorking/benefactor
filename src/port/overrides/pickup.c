/* src/port/overrides/pickup.c — native port of the object-PICKUP mechanic (widened).
 *
 ...
 * restored immediately after. The native owner has no static fallback.
 *
...
/* Per-handler X-window constants, recovered from the retail-image handlers
...
static const struct {
    ...
    {0x586B1Cu, 4, 8, OBJX_A0},  {0x586B2Au, 4, 8, OBJX_A0},    {0x586C10u, 4, 8, OBJX_A0},
    {0x586C1Eu, 4, 8, OBJX_A0},  {0x586D14u, -4, 12, OBJX_A0},  {0x586E6Cu, 0, 16, OBJX_A0},
    {0x586ECCu, 0, 16, OBJX_A0}, {0x586F9Cu, 0, 9, OBJX_A0},    {0x587006u, 4, 8, OBJX_A0},
    {0x5870CEu, 0, 16, OBJX_A0}, {0x5871F4u, 0, 16, OBJX_A0},   {0x587272u, 3, 8, OBJX_A0},
    {0x58733Au, 0, 12, OBJX_A0}, {0x58743Cu, 0, 16, OBJX_A0},   {0x5874FEu, 0, 16, OBJX_A0},
    {0x587554u, 0, 16, OBJX_A0}, {0x587616u, 0, 16, OBJX_A0},   {0x58766Eu, 0, 16, OBJX_A0},
    {0x5876C4u, 0, 16, OBJX_A0}, {0x589572u, 0, 16, OBJX_A0},   {0x589642u, -8, 16, OBJX_A0},
    {0x58A828u, 8, 16, OBJX_A0}, {0x58BF24u, -32, 48, OBJX_A0}, {0x595FE4u, 4, 10, OBJX_A0},
    {0x595FF4u, 4, 10, OBJX_D0}, {0x596064u, 4, 10, OBJX_A0},   {0x59610Au, 4, 10, OBJX_A0},
    {0x597548u, 0, 16, OBJX_A0}, {0x597892u, 4, 8, OBJX_A0},    {0x59B0B0u, -8, 20, OBJX_A0_84},
};
...
static int nudged_px(M68KCtx *ctx, uint32_t addr, int px, int extend) {
    for (unsigned i = 0; i < sizeof s_xwin / sizeof s_xwin[0]; i++) {
        ...
        switch (s_xwin[i].src) {
        case OBJX_D0:
            objX = (int16_t)ctx->D[0];
            break;
        case OBJX_A0_84:
            objX = (int16_t)MR16(ctx->A[0] + 0x84u);
            break;
        default:
...
static void interact_wide(M68KCtx *ctx, uint32_t addr, int extend, int latch) {
    ...
    if (!pc_modern_any()) {
        ...
        if (extend > 0 && (s_f80 & 0x20)) { /* fire held → reach further    */
            ...
            benefactor_log_write(BENEFACTOR_LOG_DEBUG, "override",
                                 "[interact-v] $%06X a0=%06X objX=%d objY=%d "
                                 "px=%d py=%d npx=%d\n",
                                 addr, ctx->A[0], (int)(int16_t)MR16(ctx->A[0]),
                                 (int)(int16_t)MR16(ctx->A[0] + 2u), px,
                                 (int)(int16_t)MR16(ctx->A[5] + A5_F94), npx);
        }
        rt_call_original(ctx, ctx->image, addr);
    ...
    if (latch && pc_cfg_bool("interact_scan", 0)) {
        ...
        if (!known && nseen < 32) {
            seen[nseen++] = addr;
            benefactor_log_write(BENEFACTOR_LOG_DEBUG, "override",
                                 "[interact-scan] $%06X a0=%06X objX=%d objY=%d st4=%04X\n", addr,
                                 ctx->A[0], (int)(int16_t)MR16(ctx->A[0] + 2u),
...
static void pickup_wide(M68KCtx *ctx, uint32_t addr) {
    interact_wide(ctx, addr, interact_extend_px(), 0);
}
static void lever_wide(M68KCtx *ctx, uint32_t addr) {
    interact_wide(ctx, addr, interact_extend_px(), 1);
}
...
void native_mm_pickup_gate(M68KCtx *ctx) {
    ...
    if (pc_modern_any() && !(hw_get_interact() || hw_get_fire_vanilla()))
        ctx->D[4] &= ~0x4000u;
    rt_call_original(ctx, ctx->image, 0x0057EA76u);
}
...
#define PKSCAN(hex)                                                                                \
    static void pkscan_##hex(M68KCtx *ctx) {                                                       \
        static int seen = 0;                                                                       \
        if (!seen) {                                                                               \
            seen = 1;                                                                              \
        ...
        rt_call_original(ctx, ctx->image, 0x##hex##u);                                             \
    }
PKSCAN(586B1C)
PKSCAN(586B2A)
PKSCAN(586C10)
