...
/* Native replacement for the original retail-image routine at $0074DC.
 ...
 *
 * Faithful reimplementation of the M68K routine at $74AA, which the original retail-image
 ...
 * the original retail-image hold loops use), so each pass advances the displayed frame.
 *
 ...
 * it, exactly like the original retail-image hold loops. No continuation state needed. */
 ...
 * gameplay image (g_overlay_active selects its image-qualified overrides), and enter
 * at $3330. */
...
void native_overlay_load_d0(void) {
     ...
     * behind the retained disk/image owner. It re-decrunches the boot image
 ...
 * runtime-loaded fast-RAM code that must execute through the image-qualified
 * runtime boundary. Until that boundary exists this owner cannot resume the
 * retail entry point. */
 ...
 * src/port/overrides/boot.c, where the menu item itself is changed rather than the glyphs.) */
...
void native_menu_glyph_blit(M68KCtx *ctx) {
    ...
    benefactor_log_write(BENEFACTOR_LOG_TRACE, "glyph", "a2=$%06X a1=$%06X d6=%d str=\"%s\"",
...
/* Native main-menu fire-dispatch at original address $0039D0. Reached when fire was
 ...
 *
 * This override replaces the original routine at $0039D0; the menu's per-frame
...
void native_menu_setup(M68KCtx *ctx) {
     ...
     * the pc_menu_subtext_overlay small-text subsystem (src/port/level_select_ui.c). */
 ...
 * overlay; these two never original retail-image — they were stubbed to a no-op, which
 ...
 *    it every frame via `beq.w $3daa` in the original routine at $003BAA (a
 *    interpreter jump). The following inline bytes in that retail image are
 *    unreachable (no guest branch targets them).
 *  - Why every earlier entry override "didn't fire": the runtime adapter's override lookup
...
void native_overlay_loader_reloc(M68KCtx *ctx) {
    ...
    if (ctx->D[0] == 3u) {
        ...
        benefactor_log_write(BENEFACTOR_LOG_DEBUG, "override",
                             "[overlay-loader] $150 d0=3: end-game/credits overlay loaded;"
     ...
     * gameplay bank through the image-qualified executor boundary. */
    ...
    if (g_pc_start_level > 0) {
        ...
        benefactor_log_write(BENEFACTOR_LOG_DEBUG, "override",
                             "[level-select] applying $20.w = %d at $150 hand-off\n", n);
    }
