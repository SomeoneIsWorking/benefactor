/* src/port/overrides/platformer.c — BENMOTION: native ownership of player air physics
...
static int is_air_state(uint32_t a) {
    return a == 0x579D00u || a == 0x579D0Eu || a == 0x579D1Cu || a == 0x579D2Au || a == 0x579D38u ||
           a == 0x579D46u || a == 0x579D52u || a == 0x579D84u || a == 0x579DDCu || a == 0x579A62u ||
           a == 0x579F3Au || a == 0x579E02u;
}
...
static int t_jump_vy(void) {
    return pc_cfg_int("pf_jump_vy", -768);
} /* initial vy.
-640 (~11px apex) felt like "barely leaves the
ground" (user, 2026-06-12); -768 ≈ 18px. */
static int t_air_acc(void) {
    return pc_cfg_int("pf_air_accel", 256);
} /* px/f^2 *256.
...
static int t_air_drag(void) {
    return pc_cfg_int("pf_air_drag", 192);
} /* px/f^2 *256.
...
static int pf_active(M68KCtx *ctx) { return pc_platformer_on() && MR16(0x1Eu) != 8; }
...
static int jump_input(void) { return hw_get_hop() || hw_joy_up(); }
...
static void takeoff_grunt(M68KCtx *ctx) {
    ...
    ctx->A[3] = (MR16(ctx->A[5] + 0x1098u) == 1) ? ctx->A[5] + 0x6DD4u : MR32(ctx->A[5] - 0x273Cu);
    ctx->D[6] = 0;
    rt_call(ctx, ctx->image, 0x0058656Eu);
    for (int i = 0; i < 8; i++) {
        ctx->D[i] = sd[i];
        ctx->A[i] = sa[i];
    }
...
static void wall_feedback(int x_in) {
    if (s_x_expect_valid && x_in != s_x_expect) {
        s_vx = 0;
        s_x_acc = 0;
    }
}
...
static void flight_anim(M68KCtx *ctx) {
    ...
    if (cell == 0) {
        if (s_roll) {
            s_d5 = 0;
            cell = (int16_t)MR16(tab);
        } /* loop the roll */
        else
            cell = 0; /* hold last pose */
    }
    if (cell != 0)
...
static void flight_rise(M68KCtx *ctx) {
    ...
    if (ctx->D[4] & (1u << 14)) {
        uint32_t slot = 0x5A4526u + (uint32_t)((MR16(ctx->A[5] + 0xFA2u) & 0xFu) << 6);
...
static void flight_fall(M68KCtx *ctx) {
    ...
    if (ctx->D[4] & (1u << 3)) {
        uint32_t d1 = ctx->D[1];
        rt_call(ctx, ctx->image, 0x00579A00u);
...
void native_pf_hop(M68KCtx *ctx) {
    benefactor_log_write(BENEFACTOR_LOG_DEBUG, "override", "[pf] hop  prev=%08X fly=%d vy=%d\n",
                         MR32(ctx->A[5] + 0xF78u), s_fly, s_vy);
    if (!pf_active(ctx)) {
        s_fly = s_track = 0;
        rt_call_original(ctx, ctx->image, 0x00579D84u);
        return;
    }
...
void native_pf_lj(M68KCtx *ctx) {
    if (!pf_active(ctx)) {
        s_fly = s_track = 0;
        rt_call_original(ctx, ctx->image, 0x00579A62u);
        return;
    }
    flight_start(ctx, 0);
...
void native_pf_diag(M68KCtx *ctx) {
    if (!pf_active(ctx)) {
        s_fly = s_track = 0;
        rt_call_original(ctx, ctx->image, 0x00579E02u);
        return;
    }
    flight_start(ctx, 0);
    flight_rise(ctx);
...
void native_pf_fall(M68KCtx *ctx) {
    if (!pf_active(ctx)) {
        s_fly = s_track = 0;
        rt_call_original(ctx, ctx->image, 0x00579F3Au);
        return;
    }
...
static void air_track(M68KCtx *ctx, uint32_t original_address) {
    if (!pf_active(ctx)) {
        s_fly = s_track = 0;
        rt_call_original(ctx, ctx->image, original_address);
        return;
    }
    ...
    if (sdx)
        ctx->D[1] = (ctx->D[1] & 0xFFFF0000u) | (uint16_t)(int16_t)(x_in + dxv + sdx);

    if (dxv || sdx)
        s_vx = (dxv << 8) + s_steer; /* combined momentum for fall */

    /* The arc's own hand-off to fall is kept; when it fires, mark the model
     * live so the fall entry preserves the tracked momentum. */
    if (MR32(ctx->A[5] + 0xF70u) == ST_FALL) {
        s_fly = 1;
        s_track = 0;
    }
    (void)state_in;
...
void native_pf_arc(M68KCtx *ctx) { air_track(ctx, 0x00579D52u); }
void native_pf_longjump(M68KCtx *ctx) { air_track(ctx, 0x00579DDCu); }
 ...
 * original guest body and give back half (light) or all (none) of it — the
...
void native_pf_landing_impact(M68KCtx *ctx) {
    ...
    rt_call_original(ctx, ctx->image, 0x00579F86u);
    if (mode)
...
void native_pf_collision(M68KCtx *ctx) {
    ...
    rt_call_original(ctx, ctx->image, 0x0057A934u);
    if (dmg_mode)
    ...
    benefactor_log_write(BENEFACTOR_LOG_DEBUG, "override", "[pf] trig j=%d edge=%d st=%08X\n", j,
                         edge, MR32(ctx->A[5] + 0xF70u));
    ...
    if (st != 0)
        return;
    if (MR16(ctx->A[5] + 0xF82u) == 0x14)
        return; /* vanilla's own gate */
    int x = (int16_t)MR16(ctx->A[5] + 0x10A6u);
    int y = (int16_t)MR16(ctx->A[5] + 0x10A8u);
    if (tile_nojump(ctx, x, y))
        return; /* no-jump tile (water etc.) */
