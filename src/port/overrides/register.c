/* register.c — Native override registration hub
 *
 * Implementations live in domain-specific files:
 *   hw.c     — hardware-wait eliminators, blitter init
 *   boot.c   — boot animation
...
void pc_register_overrides(void) {
     ...
     *   They were redirected to $003872 for a long time, which is why
    ...
    /* Widescreen object capture (overrides/gameplay.c): record each object's
     ...
     * the full wide view. Both call the original guest body → vanilla unaffected. */
    ...
    /* BENMOTION platformer physics (opt-in "platformer_physics", overrides/
     ...
     * three overrides — the input re-gate ($57DEAC, overrides/gameplay.c) and the
     * pickup/lever wideners (overrides/pickup.c).
     *
     ...
     * the pickup/interact override lists (overrides/pickup.c) yet; find them
     ...
     *     item handlers. */
    {
        ...
        benefactor_log_write(
            ...
            "[controls] modern_keyboard=%d modern_controller=%d interact_extend=%d\n",
            pc_modern_kb(), pc_modern_pad(), interact_extend_px());

        if (pc_cfg_bool("pickup_scan", 0)) {
