...
/* ── Threaded waits ────────────────────────────────────────────────────────────
 * The game runs on its OWN OS thread. A frame wait (hw_vblank_wait / hw_wait_fire
 * / hw_blitter_sync) is a plain BLOCKING call: it blocks the game thread on a
 * condvar until the SDL main thread releases it (once per frame). The C call
 * stack of the game thread IS the suspended state — no continuation/yield. */
