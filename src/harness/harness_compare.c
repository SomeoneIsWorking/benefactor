 ...
 */
static void report_pixel_cause(int px, int py, const uint16_t *plist, const uint16_t *clist,
                               int nwords) {
    ...
    for (int i = 0; i + 1 < nwords; i += 2) {
        ...
        if (pw1 & 1) {
            ...
            int affects_pixels = (reg >= 0x180 && reg <= 0x1BE) || /* COLORxx */
                                 reg == 0x100 ||                   /* BPLCON0 */
                                 reg == 0x102 ||                   /* BPLCON1 */
    ...
    if (cause_cop_idx >= 0) {
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                             "[pixel-cause] first diff pixel (%d,%d): "
                             "cop[%03d] line~%d %s: PUAE=$%04X PC=$%04X\n",
                             px, py, cause_cop_idx, cur_line, cause_name ? cause_name : "???",
                             cause_puae, cause_pc);
    } else {
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                             "[pixel-cause] first diff pixel (%d,%d): "
...
static void dump_suspect_state_traces(void) {
    static const struct {
        ...
        {0x003800u, 0x003806u, "phases-3800"},         {0x003B92u, 0x003C2Eu, "3B92-thresh"},
        {0x007CF4u, 0x007D50u, "title-copper-bplmod"}, {0x00419Au, 0x0041A4u, "title-swap"},
        {0x0042FCu, 0x004308u, "focus-state"},         {0x0069F0u, 0x006AEAu, "timer-state"},
        {0x07FFA2u, 0x080000u, "stack-dispatch"},
    };

    benefactor_log_write(BENEFACTOR_LOG_INFO, "harness", "[frame-trace] suspect state ranges:\n");
    for (size_t i = 0; i < sizeof(ranges) / sizeof(ranges[0]); i++) {
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness", "[frame-trace] %s $%06X-$%06X\n",
...
void compare_framebuffers(int fi) {
    ...
    benefactor_log_write(BENEFACTOR_LOG_INFO, "harness", "[pixel] Diff: %d / %d (%.1f%%)", px_diff,
                         FB_W * FB_H, 100.0 * px_diff / (FB_W * FB_H));
    if (px_diff > 0) {
        int fpx = first_px % FB_W, fpy = first_px / FB_W;
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                             "  first at (%d,%d) PUAE=%06X PC=%06X\n", fpx, fpy,
                             pf[first_px] & 0xFFFFFFu, cf[first_px] & 0xFFFFFFu);

        {
            FILE *fp = harness_artifact_open("harness_puae_fb_diff.bin", "wb");
            if (fp) {
                fwrite(pf, sizeof(uint32_t), FB_W * FB_H, fp);
                fclose(fp);
            }
            fp = harness_artifact_open("harness_pc_fb_diff.bin", "wb");
            if (fp) {
        ...
        if (pair_count > 0) {
            benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                                 "[pixel-pairs] top pairs (up to %d unique):\n", pair_count);
            for (int k = 0; k < pair_count; k++) {
                benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                                     "  PUAE=%06X PC=%06X count=%d\n", pairs[k].p, pairs[k].c,
                                     pairs[k].n);
            }
        }
        ...
        if (fi < (int)(sizeof(s_puae_log) / sizeof(s_puae_log[0])) &&
            ...
            report_pixel_cause(fpx, fpy, s_puae_log[fi].coplist, s_pc_log[fi].coplist,
        ...
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness", "  MATCH ✓\n");
    }
}
...
void compare_phases(int puae_frames, int pc_frames, const char *report_path) {
    ...
    if (first_diff < 0) {
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                             "[harness] PERFECT MATCH across all %d frames\n", n);
        return;
    }

    benefactor_log_write(BENEFACTOR_LOG_INFO, "harness", "[harness] First divergence at frame %d\n",
    ...
    benefactor_log_write(BENEFACTOR_LOG_INFO, "harness", "  cop1lc  PUAE=$%06X  PC=$%06X  %s\n",
...
static void compare_chipram_full(void) {
    ...
    for (int i = 0; i <= CHIP_RAM_SIZE; i++) {
        int differs = (i < CHIP_RAM_SIZE) && (s_puae_chipram_snap[i] != s_pc_chipram_snap[i]);
        if (differs) {
            if (range_start < 0) {
                range_start = i;
                range_count = 0;
            ...
            if (n_ranges < 64) {
                ...
                benefactor_log_write(
                    ...
                    "[chipram-diff]   $%06X–$%06X (%d byte%s)  PUAE=%02X  PC=%02X\n", range_start,
                    end, range_count, range_count > 1 ? "s" : "", s_puae_chipram_snap[range_start],
                    s_pc_chipram_snap[range_start]);
            } else if (n_ranges == 64) {
                benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                                     "[chipram-diff]   ... (truncated at 64 ranges)\n");
            }
            n_ranges++;
            range_start = -1;
        }
...
void compare_bitplanes(const FrameState *p) {
    if (!s_puae_chipram_valid || !s_pc_chipram_valid) {
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                             "[bpl-diff] chip RAM snapshots not available\n");
        return;
    }
    if (!p->coplist_valid) {
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                             "[bpl-diff] PUAE copper list not captured\n");
        return;
    }
    ...
    if (s_pc_prerender_bpl_crc_valid) {
        ...
        benefactor_log_write(
            BENEFACTOR_LOG_INFO, "harness",
            "[bpl-prerender] PC pre-render BPL CRC=$%08X  PUAE post-frame BPL CRC=$%08X  %s\n",
    ...
    benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                         "[bpl-diff] BPL1MOD=%d BPL2MOD=%d  fetch=%d bytes/line  height=%d\n",
    ...
    for (int pass = 0; pass < 2; pass++) {
        ...
        for (int pl = 0; pl < 4; pl++) {
            ...
            if (first_off < 0) {
                benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                                     "[bpl-diff] BPL%d (%s) @ $%06X–$%06X: MATCH\n", pl + 1,
                                     pass_label, base, base + (uint32_t)scan_bytes - 1);
            } else {
                ...
                benefactor_log_write(
                    BENEFACTOR_LOG_INFO, "harness",
                    "[bpl-diff] BPL%d (%s) @ $%06X: first diff +%d ($%06X) line=%d col=%d  "
                ...
                benefactor_log_write(
                    BENEFACTOR_LOG_INFO, "harness",
                    "[bpl-trace] Writes near first diff for BPL%d (%s) ($%06X-$%06X):\n", pl + 1,
                    pass_label, da >= 0x10u ? da - 0x10u : 0u, da + 0x10u);
                trace_dump_range(stdout, da >= 0x10u ? da - 0x10u : 0u, da + 0x11u, 32);
            }
        }
    }
    ...
    benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                         "[chipram-diff] Full chip RAM diff (post-frame):\n");
    compare_chipram_full();

    /* Save both snapshots so the Python analysis script can read them */
    {
        FILE *fp;
        fp = harness_artifact_open("harness_puae_after_frame.bin", "wb");
        if (fp) {
            fwrite(s_puae_chipram_snap, 1, CHIP_RAM_SIZE, fp);
            fclose(fp);
        }
        fp = harness_artifact_open("harness_pc_after_frame.bin", "wb");
        if (fp) {
            fwrite(s_pc_chipram_snap, 1, CHIP_RAM_SIZE, fp);
            fclose(fp);
        }
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness",
                             "[bpl-diff] Saved chip RAM snapshots in harness scratch\n");
    }
}
