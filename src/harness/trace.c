...
void trace_dump_side_by_side(FILE *fp, uint32_t addr, int max_entries) {
    ...
    for (int i = 0; i < limit; i++) {
        ...
        snprintf(pl, sizeof(pl), "%s%04X->%04X pc=%06X sz=%d", pblit, p->old_val, p->new_val, p->pc,
                 p->size);
        snprintf(cl, sizeof(cl), "%s%04X->%04X pc=%06X sz=%d", cblit, c->old_val, c->new_val, c->pc,
                 c->size);
        fprintf(fp, "  %-50s | %-50s\n", pl, cl);
    }
