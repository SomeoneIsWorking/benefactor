...
/* Bounded diagnostic: log who writes the $4d064 level table (CPU vs blitter). */
...
static int _puae_is_state_watch_addr(uaecptr addr) {
    return (addr >= 0x0042FCu && addr < 0x0042FEu) || (addr >= 0x0069F0u && addr < 0x006AEAu) ||
           (addr >= 0x07FFA2u && addr < 0x080000u);
}

static int _puae_is_timer_selector_addr(uaecptr addr) {
    return (addr >= 0x0069F0u && addr < 0x006AEAu) || (addr >= 0x07FFA2u && addr < 0x080000u);
}

static int _puae_is_focus_addr(uaecptr addr) {
    return (addr >= 0x0042FCu && addr < 0x004308u) || (addr >= 0x0069F1u && addr < 0x0069FEu) ||
           (addr >= 0x006A27u && addr < 0x006AEAu) || (addr >= 0x07FFA2u && addr < 0x080000u);
}

static int _puae_is_bpl_addr(uaecptr addr) {
    return (addr >= 0x04F6F4u && addr < 0x0546F4u) || (addr >= 0x070930u && addr < 0x075930u) ||
           (addr >= 0x04F71Cu && addr < 0x05471Cu) || (addr >= 0x025370u && addr < 0x02A370u);
}

/* Address ranges we care about: copper list ($86CC-$88CC) and $7Cxx tables */
static int _puae_addr_interesting(uaecptr addr) {
    return (addr >= 0x86CCu && addr < 0x88CCu) || (addr >= 0x7BC8u && addr < 0x7E00u) ||
           /* Focused bitplane windows around current first-diff bytes */
           (addr >= 0x04F74Bu && addr < 0x04F76Cu) || (addr >= 0x070924u && addr < 0x070945u) ||
           (addr >= 0x025387u && addr < 0x0253A8u) ||
           /* Upstream source-chain windows feeding those writes */
           (addr >= 0x048240u && addr < 0x048260u) || (addr >= 0x069300u && addr < 0x069500u) ||
           (addr >= 0x061E80u && addr < 0x061EC0u) || _puae_is_state_watch_addr(addr);
}
    ...
    if (orig_chipmem_wput) {
        if (_puae_addr_interesting(addr)) {
            uint16_t old_v = (addr + 1 < chipmem_bank.allocated_size)
                                 ? (uint16_t)((chipmem_bank.baseaddr[addr] << 8) |
                                              chipmem_bank.baseaddr[addr + 1])
                                 : 0;
...
void puae_snap_state(FrameState *s) {
    ...
    for (i = 0; i < 32; i++) {
        ...
        if (aga_mode) {
            ...
            s->palette[i] = (uint16_t)((((aga >> 16) & 0xF) << 8) | (((aga >> 8) & 0xF) << 4) |
                                       ((aga >> 4) & 0xF));
        } else
#endif
