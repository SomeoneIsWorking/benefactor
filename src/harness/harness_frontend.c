 ...
 *
 * Libretro frontend for the separately composed PUAE oracle harness.
...
static void video_cb(const void *data, unsigned w, unsigned h, size_t pitch) {
    ...
    if (!s_logged_geom) {
        ...
        benefactor_log_write(BENEFACTOR_LOG_DEBUG, "harness",
                             "[PUAE_FB_GEOM] w=%u h=%u pitch=%zu bpp=%d", w, h, pitch, s_puae_bpp);
    }
    if (!s_crop_inited) {
        s_crop_inited = 1;
        s_crop_w = (int)w;
        ...
        s_crop_x = 100;
        s_crop_w = 704;
        s_crop_x = pc_cfg_int("puae_crop_x", s_crop_x);
...
/* Audio capture (BENEFACTOR_AUDIO_DUMP): PUAE PCM -> the harness scratch activity (stereo int16
 * at PUAE's libretro rate). 'frames' is sample-frames (L+R pairs). */
static size_t audio_batch_cb(const int16_t *data, size_t frames) {
    if (data && frames && pc_cfg_bool("audio_dump", 0) && !g_harness_fast_forward) {
        static FILE *s_apu = NULL;
        static int s_init = 0;
        if (!s_init) {
...
static void audio_cb(int16_t l, int16_t r) {
    if (pc_cfg_bool("audio_dump", 0) && !g_harness_fast_forward) {
        static FILE *s_apu1 = NULL;
        static int s_init = 0;
        if (!s_init) {
            s_apu1 = harness_artifact_open("audio_puae_single.raw", "wb");
            s_init = 1;
        }
        if (s_apu1) {
            int16_t lr[2] = {l, r};
            fwrite(lr, sizeof(int16_t), 2, s_apu1);
        }
    }
}
...
static int16_t input_state_cb(unsigned port, unsigned dev, unsigned idx, unsigned id) {
    ...
    case RETRO_DEVICE_ID_MOUSE_LEFT:
        return input_space(); /* port-0 (mouse) button */
    default:
        return 0;
    }
}
...
void harness_combined_init(void) {
    ...
    s_sdl_tex = SDL_CreateTexture(s_sdl_ren, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING,
                                  win_w, win_h);
    if (!s_sdl_tex) {
        benefactor_log_write(BENEFACTOR_LOG_INFO, "harness", "[sdl] CreateTexture: %s\n",
                             SDL_GetError());
        return;
    }
    s_sdl_win_w = win_w;
