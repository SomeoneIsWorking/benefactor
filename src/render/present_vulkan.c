 ...
 *      Invoke through the future product's named Vulkan self-test mode.
 *
 ...
 *
 * See instructions/rendering-overhaul-plan.md + docs/codemap.md. */
...
#define VKLOG(...) benefactor_log_write(BENEFACTOR_LOG_ERROR, "vulkan", __VA_ARGS__)
#define VK_OK(call)                                                                                \
    do {                                                                                           \
        VkResult _r = (call);                                                                      \
        if (_r != VK_SUCCESS) {                                                                    \
...
static int vk_offscreen_render(const uint32_t *argb, int w, int h, uint32_t *out) {
    ...
    uint32_t qfam = UINT32_MAX;
    {
        ...
        if (pd == VK_NULL_HANDLE) {
            VKLOG("no graphics-capable device\n");
            goto fail;
        }
        VkPhysicalDeviceProperties props;
        vkGetPhysicalDeviceProperties(pd, &props);
        benefactor_log_write(BENEFACTOR_LOG_INFO, "vulkan", "device: %s", props.deviceName);
    }
...
#define MK_IMAGE(img, mem, view, _usg)                                                             \
    do {                                                                                           \
        ...
        VkMemoryAllocateInfo mai = {                                                               \
            ...
            .memoryTypeIndex =                                                                     \
                find_mem_type(pd, mr.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT)};        \
...
#define MK_BUFFER(buf, mem, _usg)                                                                  \
    do {                                                                                           \
        ...
        VkMemoryAllocateInfo mai = {.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,               \
                                    .allocationSize = mr.size,                                     \
                                    .memoryTypeIndex =                                             \
                                        find_mem_type(pd, mr.memoryTypeBits,                       \
...
#undef MK_BUFFER
    ...
    {
        void *p;
        VK_OK(vkMapMemory(dev, up_mem, 0, img_bytes, 0, &p));
        memcpy(p, argb, img_bytes);
        vkUnmapMemory(dev, up_mem);
    }

    /* ── render pass (one color attachment, final layout TRANSFER_SRC) ─────── */
    {
        VkAttachmentDescription at = {.format = FMT,
                                      .samples = VK_SAMPLE_COUNT_1_BIT,
    ...
    /* ── shaders, descriptor set, pipeline ────────────────────────────────── */
    {
        ...
        VkShaderModuleCreateInfo fi = {.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,
                                       .codeSize = sizeof k_blit_frag_spv,
                                       .pCode = k_blit_frag_spv};
    ...
    VkDescriptorSet dset;
    {
        ...
        VkDescriptorImageInfo dii = {.sampler = samp,
                                     .imageView = src_view,
    ...
    VkCommandBuffer cb;
    {
        ...
        vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_TRANSFER_BIT,
                             VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, NULL, 0, NULL, 1, &b2);
    ...
    {
        void *p;
        VK_OK(vkMapMemory(dev, rb_mem, 0, img_bytes, 0, &p));
        memcpy(out, p, img_bytes);
...
static int sw_make_swapchain(Swap *s, uint32_t w, uint32_t h) {
    ...
    uint32_t nf = 0;
    vkGetPhysicalDeviceSurfaceFormatsKHR(s->pd, s->surface, &nf, NULL);
    ...
    VkSwapchainCreateInfoKHR ci = {.sType = VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR,
                                   ...
                                   .imageArrayLayers = 1,
                                   .imageUsage =
                                       VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, /* we render into it */
...
static void vk_point_desc(Swap *s, VkDescriptorSet set, VkImageView view) {
    VkDescriptorImageInfo dii = {.sampler = s->samp,
                                 .imageView = view,
                                 .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
    VkWriteDescriptorSet wr = {.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
                               .dstSet = set,
...
static int vk_make_render_pass(Swap *s) {
    VkAttachmentDescription at = {.format = s->sc_format,
                                  .samples = VK_SAMPLE_COUNT_1_BIT,
                                  .loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR,
                                  .storeOp = VK_ATTACHMENT_STORE_OP_STORE,
                                  .stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE,
...
static int vk_make_pipeline(Swap *s) {
    ...
    VkPipelineColorBlendAttachmentState cba_a = {
        .blendEnable = VK_TRUE,
        ...
        .colorWriteMask = 0xF};
    VkPipelineColorBlendStateCreateInfo cb_a = {
        .sType = VK_STRUCTURE_TYPE_PIPELINE_COLOR_BLEND_STATE_CREATE_INFO,
        .attachmentCount = 1,
        .pAttachments = &cba_a};
...
static int vulkan_init(const char *title, int cw, int ch) {
    ...
    uint32_t nd = 0;
    vkEnumeratePhysicalDevices(s->inst, &nd, NULL);
    ...
    free(devs);
    if (s->pd == VK_NULL_HANDLE) {
        VKLOG("no graphics+present device\n");
        goto fail;
    }
    {
        VkPhysicalDeviceProperties p;
        vkGetPhysicalDeviceProperties(s->pd, &p);
        benefactor_log_write(BENEFACTOR_LOG_INFO, "vulkan", "windowed device: %s", p.deviceName);
    }

    {
        ...
        if (vkCreateDevice(s->pd, &di, NULL, &s->dev) != VK_SUCCESS) {
            VKLOG("vkCreateDevice\n");
            goto fail;
        }
        vkGetDeviceQueue(s->dev, s->qfam, 0, &s->queue);
    }

    if (sw_make_swapchain(s, (uint32_t)cw, (uint32_t)ch) != 0) {
        VKLOG("swapchain\n");
        goto fail;
    }
    if (vk_make_render_pass(s) != 0) {
        VKLOG("render pass\n");
        goto fail;
    }
    if (sw_make_targets(s) != 0) {
        VKLOG("framebuffers\n");
        goto fail;
    }
    ...
    if (vk_make_pipeline(s) != 0) {
        VKLOG("pipeline\n");
        goto fail;
    }
    if (vk_make_fx_pipelines(s) != 0) {
        VKLOG("fx pipelines\n");
        goto fail;
    }
    ...
    if (vk_tex_make(s, &s->atlas, VK_ATLAS_W, VK_ATLAS_H) != 0) {
        VKLOG("atlas\n");
        goto fail;
    }
...
static void sw_barrier(VkCommandBuffer cb, VkImage img, VkImageLayout from, VkImageLayout to,
                       VkAccessFlags sa, VkAccessFlags da, VkPipelineStageFlags ss,
...
static void vk_tex_upload_cmd(Swap *s, VkTex *t) {
    VkImageLayout from =
        t->uploaded ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_UNDEFINED;
    VkAccessFlags sa = t->uploaded ? VK_ACCESS_SHADER_READ_BIT : 0;
    VkPipelineStageFlags ss =
        t->uploaded ? VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT : VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
    sw_barrier(s->cmd, t->img, from, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, sa,
               VK_ACCESS_TRANSFER_WRITE_BIT, ss, VK_PIPELINE_STAGE_TRANSFER_BIT);
    ...
    sw_barrier(s->cmd, t->img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
static void vk_draw_quad(Swap *s, VkDescriptorSet set, const QuadPush *qp) {
    ...
    vkCmdPushConstants(s->cmd, s->pll, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                       sizeof *qp, qp);
...
static int vk_atlas_pack_bake(Swap *s, const Scene *sc, AtlasPos *pos) {
    ...
    for (int i = 0; i < sc->nquads; i++) {
        ...
        for (int rr = 0; rr < q->h; rr++) {
            int dy = q->y + rr;
            const uint32_t *pal =
                (dy >= 0 && dy < SCENE_MAX_ROWS) ? sc->pal_rows[dy] : sc->pal_rows[0];
...
static int vk_begin_frame(Swap *s, uint32_t *out_idx) {
    ...
    uint32_t idx = 0;
    VkResult ar =
        vkAcquireNextImageKHR(s->dev, s->swap, UINT64_MAX, s->sem_acquire, VK_NULL_HANDLE, &idx);
...
static void vk_end_frame(Swap *s, uint32_t idx) {
    ...
    VkSubmitInfo subm = {.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO,
                         ...
                         .pWaitSemaphores = &s->sem_acquire,
                         .pWaitDstStageMask = &wait_stage,
                         .commandBufferCount = 1,
                         .pCommandBuffers = &s->cmd,
                         .signalSemaphoreCount = 1,
                         .pSignalSemaphores = &s->sem_done[idx]};
    vkQueueSubmit(s->queue, 1, &subm, s->fence);
    VkPresentInfoKHR pi = {.sType = VK_STRUCTURE_TYPE_PRESENT_INFO_KHR,
                           .waitSemaphoreCount = 1,
                           .pWaitSemaphores = &s->sem_done[idx],
                           .swapchainCount = 1,
                           .pSwapchains = &s->swap,
                           .pImageIndices = &idx};
...
static void vk_begin_pass(Swap *s, uint32_t idx, int cw, int ch) {
    ...
    if (ew / eh > ca) {
        vh = eh;
        vw = eh * ca;
    } /* window wider → pillarbox (L/R bars) */
    else {
        vw = ew;
        vh = ew / ca;
    } /* window taller → letterbox (T/B bars) */
    s->vp[0] = (ew - vw) * 0.5f;
    s->vp[1] = (eh - vh) * 0.5f;
    s->vp[2] = vw;
    s->vp[3] = vh;
...
static void vulkan_present_scene(const Scene *sc, int y_lo, int y_hi, const uint32_t *base, int w,
    ...
    if (y_hi < h) {
        ...
        q.dst[1] = (float)y_hi;
        q.dst[2] = (float)w;
        q.dst[3] = (float)(h - y_hi);
        q.src[1] = (float)y_hi / (float)h;
        q.src[2] = 1.0f;
        q.src[3] = 1.0f;
        vk_draw_quad(s, s->dset_base, &q);
    }
        ...
        if (cx1 > cx0) {
            ...
            for (int i = 0; i < sc->nquads; i++) {
                ...
                if (shadow_on && q->shadow) {
                    ...
                    qs.dst[0] = (float)(q->x - sc->view_left) + SH_DX;
                    qs.dst[1] = (float)q->y + SH_DY;
                    qs.dst[2] = (float)q->w;
                    qs.dst[3] = (float)q->h;
                    qs.src[0] = u0;
                    qs.src[1] = v0;
                ...
                qp.screen[0] = scr0;
                qp.screen[1] = scr1;
                qp.dst[0] = (float)(q->x - sc->view_left);
                qp.dst[1] = (float)q->y;
                qp.dst[2] = (float)q->w;
                qp.dst[3] = (float)q->h;
                qp.src[0] = u0;
     ...
     * world, BEFORE the banner so screen-fixed UI stays bright. */
    {
        ...
        if (fl & FX_AMBIENT) {
            ...
            fp.res[0] = scr0;
            fp.res[1] = scr1;
            fp.light[0] = (float)s->fx.light_sx;
            fp.light[1] = (float)s->fx.light_sy;
            fp.pf[0] = (float)s->fx.pf_top;
            fp.pf[1] = (float)s->fx.pf_bot;
            fp.flags = fl;
            fp.mode = 0;
    ...
    vkCmdBindPipeline(s->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                      s->pipe); /* restore the quad pipeline */
    vk_scissor_content(s, w, h, 0, 0, w, h);
    for (int i = 0; i < sc->nquads; i++) {
        const SceneQuad *q = &sc->quads[i];
        ...
        qp.dst[2] = (float)q->w;
        qp.dst[3] = (float)q->h;
        qp.src[0] = (float)pos[i].x / VK_ATLAS_W;
        qp.src[1] = (float)pos[i].y / VK_ATLAS_H;
        qp.src[2] = (float)(pos[i].x + q->w) / VK_ATLAS_W;
        qp.src[3] = (float)(pos[i].y + q->h) / VK_ATLAS_H;
        vk_draw_quad(s, s->dset_atlas, &qp);
    }
    ...
    for (int i = 0; i < nrects; i++) {
        ...
        qp.dst[2] = (float)rw;
        qp.dst[3] = (float)rh;
        qp.src[0] = (float)x0 / (float)w;
        qp.src[1] = (float)y0 / (float)h;
        qp.src[2] = (float)(x0 + rw) / (float)w;
        qp.src[3] = (float)(y0 + rh) / (float)h;
...
static const PresentBackend VULKAN_BACKEND = {
    ...
    vulkan_set_effects, vulkan_window, vulkan_shutdown};
