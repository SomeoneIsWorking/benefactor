#version 450
/* fx.frag — BenRen VK DIM pass (ambient darkness + torch glow). Drawn as a
 * fullscreen triangle after the world sprites, in the same render pass, with a
 * MULTIPLY blend (dst *= brightness). Outputs 1.0 (identity) outside the playfield
 * rows or when those flags are off, so it never touches the HUD/border/banner.
 * (The per-sprite back-glow is a separate additive pass in the quad pipeline.)
 *
 * Pure per-pixel + the projected light position (push constants) — no texture read,
 * no descriptor set. Geometry in CONTENT px: v_uv*res matches the scene. */
...
layout(push_constant) uniform PC {
    ...
    int  flags;   /* FX_AMBIENT|FX_TORCH */
    int  mode;    /* unused (kept for push-constant layout parity) */
} pc;
