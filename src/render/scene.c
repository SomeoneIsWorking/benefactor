...
void scene_reset(Scene *s) {
    ...
    s->shadow_next = 0;
    if (!s->arena)
        s->arena = malloc(SCENE_ARENA_BYTES);
}

void scene_set_pal_rows(Scene *s, const uint32_t *src, int nrows) {
    if (nrows > SCENE_MAX_ROWS)
        nrows = SCENE_MAX_ROWS;
...
uint8_t *scene_alloc_idx(Scene *s, uint32_t nbytes) {
    ...
    nbytes = (nbytes + 3u) & ~3u;
    if (s->arena_used + nbytes > SCENE_ARENA_BYTES)
        return NULL;
...
static void scene_add_quad_space(Scene *s, int x, int y, int w, int h, const uint8_t *idx,
                                 int stride, uint8_t space) {
    if (s->nquads >= SCENE_MAX_QUADS || !idx)
        return;
    ...
    q->stride = stride;
    q->space = space;
    /* Only world-space character sprites cast a shadow (never screen-fixed UI). */
    q->shadow = (space == SCENE_SPACE_WORLD) ? s->shadow_next : 0;
}

void scene_add_quad(Scene *s, int x, int y, int w, int h, const uint8_t *idx, int stride) {
    scene_add_quad_space(s, x, y, w, h, idx, stride, SCENE_SPACE_WORLD);
}

void scene_add_quad_screen(Scene *s, int x, int y, int w, int h, const uint8_t *idx, int stride) {
...
void scene_composite_screen_argb(const Scene *s, uint32_t *dst, int dst_w, int dst_h) {
    for (int i = 0; i < s->nquads; i++) {
        const SceneQuad *q = &s->quads[i];
        if (q->space != SCENE_SPACE_SCREEN)
            continue;
        for (int r = 0; r < q->h; r++) {
            int dy = q->y + r;
            if (dy < 0 || dy >= dst_h)
